function getNodeTree(node: BaseNode): any {
  const result: any = {
    id: node.id,
    name: node.name,
    type: node.type,
  ...
  if (node.type === "INSTANCE") {
    result.componentId = node.mainComponent?.id || null;
    result.componentName = node.mainComponent?.name || null;
  }
...
function isValidNode(node: BaseNode | null): boolean {
